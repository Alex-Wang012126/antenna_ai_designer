...
job_kimi_k3.yaml（可用 MODEL_CONFIG_YAML 指定其他文件）读取。
"""
...
def _load_model_config_from_yaml() -> Dict[str, Any]:
    ...
    """
    path = Path(__file__).resolve().parent / os.getenv("MODEL_CONFIG_YAML", "job_kimi_k3.yaml")
    if not path.is_file():
        return {}
    try:
        import yaml
...
class Config:
    ...
    model_base_url: str = (
        os.getenv("MODEL_BASE_URL") or _yaml_model.get("model_base_url") or "https://api.moonshot.cn/v1"
    )
    model_name: str = os.getenv("MODEL_NAME") or _yaml_model.get("model_name") or "kimi-k3"
